...
const StateModal: FunctionComponent<StateModalProps> = ({
  state,
  exclude,
  url,
  title,
  ...
  return (
    <Modal
      trigger={(open) => (
        <button
          className={clx(
            "border-slate-200 dark:border-zinc-700 shadow-2xl dark:bg-zinc-900 fixed bottom-0 right-4 z-30 flex h-14 w-14 transform items-center justify-center rounded-full border bg-white transition-all lg:hidden",
    ...
    >
      {(close) => (
        <ul className="dark:bg-zinc-900 max-h-96 overflow-y-auto bg-white">
          {statesOptions
                ...
                >
                  <div className="flex gap-2">
                    <Image
                      ...
                      src={"/static/images/states/".concat(
                        state.value,
                        ".jpeg",
                      )}
                      height={16}
