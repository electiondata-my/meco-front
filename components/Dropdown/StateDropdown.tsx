...
const StateDropdown: FunctionComponent<StateDropdownProps> = ({
  ...
  const redirect = (selected: OptionType) => {
    if (selected.value === "mys") {
      url && router.replace(url, undefined, { scroll: false });
      return;
    }
    url &&
      router.replace(`${url}/${selected.value}`, undefined, { scroll: false });
  };
  ...
  return (
    <div
      className={clx(
        !hideOnScroll ? `block ${width}` : show ? "hidden lg:block" : "hidden",
      )}
    >
      <Dropdown
        className={clx("flex-row items-center", className)}
        onChange={(selected) =>
          onChange ? onChange(selected) : redirect(selected)
        }
        disabled={disabled}
