...
import type { Param } from "./ParamTable";
...
const EndpointCard: FunctionComponent<EndpointCardProps> = ({ method, url, params }) => (
      ...
      <table className="w-full border-t border-otl-gray-200 text-body-sm">
        <thead>
          <tr className="border-b border-otl-gray-200 bg-bg-white text-left">
            <th className="px-4 py-3 font-semibold text-txt-black-700">Parameter</th>
            <th className="px-4 py-3 font-semibold text-txt-black-700">Type</th>
            <th className="px-4 py-3 font-semibold text-txt-black-700">Required</th>
        ...
        <tbody className="divide-y divide-otl-gray-200">
          {params.map(param => (
            <tr key={param.name} className="bg-bg-white hover:bg-bg-washed">
              ...
              <td className="px-4 py-3 font-mono text-body-xs text-txt-black-500">{param.type}</td>
              <td className="px-4 py-3">
                {param.required ? (
                  <span className="rounded bg-bg-danger-100 px-1.5 py-0.5 text-body-xs font-semibold text-txt-danger">Yes</span>
                ) : (
                  <span className="rounded bg-bg-black-100 px-1.5 py-0.5 text-body-xs font-semibold text-txt-black-500">No</span>
                )}
              </td>
              <td className="px-4 py-3 text-txt-black-600">{param.description}</td>
            </tr>
          ))}
        </tbody>
      </table>
