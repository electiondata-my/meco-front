...
export default function SignInForm() {
  ...
  useEffect(() => {
    fetch(`${AUTH_URL}/me`, { credentials: "include" })
      .then((r) => {
        if (r.ok) window.location.replace("/console");
        else setCheckingSession(false);
      })
      .catch(() => setCheckingSession(false));
  }, []);

  // Load Turnstile script and initialise widget dynamically.
  // Must wait for checkingSession=false so #cf-turnstile is in the DOM.
  useEffect(() => {
  ...
  const handleOtpKeyDown = (
    ...
    if (e.key === "Backspace" && !otp[index] && index > 0) focusOtpInput(index - 1);
    if (e.key === "ArrowLeft" && index > 0) { e.preventDefault(); focusOtpInput(index - 1); }
    if (e.key === "ArrowRight" && index < OTP_LENGTH - 1) { e.preventDefault(); focusOtpInput(index + 1); }
  };
  ...
  const handlePasteOtp = async () => {
    try {
      ...
      if (!digits) { setError("Clipboard does not contain a code."); return; }
            ...
            <>
              ...
              {step === "email" ? (
                <form onSubmit={handleEmailSubmit} className="flex flex-col gap-4">
                  ...
                  {error && <p className="text-body-sm text-txt-danger">{error}</p>}
