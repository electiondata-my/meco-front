...
const CatalogueTimeseries = dynamic(
  () => import("datagovmy-ui/dc-charts/timeseries"),
  {
    ssr: false,
  },
);
const CatalogueChoropleth = dynamic(
  () => import("datagovmy-ui/dc-charts/choropleth"),
  {
    ssr: false,
  },
);
const CatalogueGeoChoropleth = dynamic(
  () => import("datagovmy-ui/dc-charts/geochoropleth"),
  {
    ssr: false,
  },
);
const CatalogueScatter = dynamic(
  () => import("datagovmy-ui/dc-charts/scatter"),
  {
    ssr: false,
  },
);
const CatalogueMapPlot = dynamic(
  () => import("datagovmy-ui/dc-charts/mapplot"),
  {
    ssr: false,
  },
);
const CatalogueGeojson = dynamic(
  () => import("datagovmy-ui/dc-charts/geojson"),
  {
    ssr: false,
  },
);
...
const CataloguePyramid = dynamic(
  () => import("datagovmy-ui/dc-charts/pyramid"),
  {
    ssr: false,
  },
);
const CatalogueHeatmap = dynamic(
  () => import("datagovmy-ui/dc-charts/heatmap"),
  {
    ssr: false,
  },
);
...
const DCChartsAndTable: FunctionComponent<ChartTableProps> = ({
  ...
  const renderChart = (): ReactNode | undefined => {
    switch (dataset.type) {
      ...
      case "PYRAMID":
        return (
          <CataloguePyramid config={config} translations={data.translations} />
        );
      case "HEATTABLE":
        return (
          <CatalogueHeatmap config={config} translations={data.translations} />
        );
      case "SCATTER":
        return (
          <CatalogueScatter
      ...
      case "LINE":
        return (
          <CatalogueLine config={config} translations={data.translations} />
        );
      default:
        break;
    }
          ...
          (item, key) => {
            if (key === "x")
              return toDate(
                item[key],
                SHORT_PERIOD_FORMAT[
                  data.frequency as keyof typeof SHORT_PERIOD_FORMAT
                ],
                i18n.language,
              );
          ...
          (item, key) => {
            if (key === "x")
              return toDate(
                item[key],
                SHORT_PERIOD_FORMAT[
                  "INTRADAY" as keyof typeof SHORT_PERIOD_FORMAT
                ],
                i18n.language,
              );
            else return item[key];
  ...
  const handleDownload = (e: { value: string }) => {
    ...
    window.open(
      e.value === "csv" ? data.link_csv : data.link_parquet,
      "_blank",
    );
    action.href();
  };
    ...
    <>
      ...
      >
        ...
        <p className="text-dim flex justify-end gap-2 py-6 text-sm">
          <span>
            {`${numFormat(result?.total_views ?? 0, "compact")} ${t(
              "common:common.views",
              {
          ...
          <span>&middot;</span>
          <span>
            {`${numFormat(result?.total_downloads ?? 0, "compact")} ${t(
