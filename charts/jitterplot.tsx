...
type JitterplotsProps = Pick<ChartHeaderProps, "title"> & {
  className?: string;
...
/** -----------------------INDIVIDUAL----------------------------------- */
type JitterplotProps = ChartHeaderProps & {
  data: JitterData;
  active: string;
  actives: string[];
  formatTitle?: (key: string) => ReactNode;
      ...
      {data.data[0].x !== null && (
        <div className="grid w-full grid-cols-1 items-center gap-1 lg:grid-cols-4">
          ...
          <div className="col-span-1 overflow-visible lg:col-span-3">
            <Bubble
              className="bg-slate-200/20 dark:border-zinc-700 h-10 overflow-visible rounded-full border px-4"
              options={options}
              data={{
                datasets: [
                  {
